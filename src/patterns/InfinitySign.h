...
public:
    void run();
    static constexpr const char *NAME = "Infinity Sign";
};

void InfinitySign::run() {
    int8_t speed = 25 * State::speed;
...
//    p_leds[XY(beatsin8(speed, 0, WIDTH - 1) - 2, beatsin8(speed * 2, HEIGHT / 4, HEIGHT - 1 - HEIGHT / 4))] += color.subtractFromRGB(4);
    ...
    p_leds[XY(beatsin8(speed, 0, WIDTH - 1) + 1,
              beatsin8(speed * 2, HEIGHT / 4, HEIGHT - 1 - HEIGHT / 4) + 1)] += color;
    p_leds[XY(beatsin8(speed, 0, WIDTH - 1) - 1,
              beatsin8(speed * 2, HEIGHT / 4, HEIGHT - 1 - HEIGHT / 4) - 1)] += color;
    p_leds[XY(beatsin8(speed, 0, WIDTH - 1) - 1,
              beatsin8(speed * 2, HEIGHT / 4, HEIGHT - 1 - HEIGHT / 4) + 1)] += color;
